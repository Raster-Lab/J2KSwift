...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.22.0 ships **decoder format-flexibility** — `J2KDecoder.decodeAnyFormat(_:Data)` + `J2KDecoder.decodeFile(at:URL)` extensions (in the `J2KFileFormat` module) that auto-detect raw J2K codestream vs JP2/JPX/JPM/JPH box-wrapped bytes and decode either; plus `J2KFileReader.extractCodestream(from:Data)` is now public (was private). Before v10.22.0, `J2KDecoder.decode(_:)` only accepted raw codestream bytes — most `.jp2` / `.jph` / `.jpx` files in the wild are JP2-wrapped (the codestream sits inside a `jp2c` Contiguous Codestream box) so consumers had to roll their own box walker. Existing `decode(_:Data)` semantics preserved — the new methods are opt-in additive surface. MINOR / additive — no behaviour change on existing paths, codestream bytes byte-identical to v10.21.0.
**Previous Release**: 10.21.0 (J2KDICOMHelpers Phase 3 — DICOM file parser)
