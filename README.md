...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.20.0 ships **JP3D `preWarm()` symmetric completion** — six new `static func preWarm(includeWarmupDispatch:) async` extensions on `JP3DEncoder`, `JP3DMultiSpectralDecoder`, `JP3DMultiSpectralEncoder`, `JP3DProgressiveDecoder`, `JP3DStreamWriter`, and `JP3DTranscoder`. Closes a discoverability gap left by v10.15.0 (which added `preWarm` only to `JP3DDecoder` + `JP3DROIDecoder`): now every JP3D actor's API surface exposes the same wrapper, so consumers using any one of these types alone can find `preWarm` without importing `J2KCodec`. All six wrappers delegate to the same `J2KDecoder.preWarm(includeWarmupDispatch:)` that warms the process-wide `J2KMetalSession.processShared`; per V10_27's earlier probe the encoder-specific leftover cold cost is below the 3 ms gate (+0.10/-1.25 ms), so this is honest discoverability, not a new perf claim. MINOR / additive — six 2-line static wrappers, codestream bytes byte-identical to v10.19.0.
**Previous Release**: 10.19.0 (J2KDICOMHelpers Phase 2 — Pixel Data encapsulation)
