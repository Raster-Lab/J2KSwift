...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.24.1 is a **patch** fixing an HTJ2K **lossless** encoder data-loss bug found by testing against the Cloudinary Image Dataset '22 (CID22): on high-contrast content the conformant HT encoder under-sized the reversible 5/3 magnitude window (`K_max`), overflowing the sign bit and silently dropping the top bitplane. The window is now sized to OpenJPH's proven-sufficient reversible `K_max`. The default Part-1 (EBCOT) encoder is unaffected and byte-identical to v10.24.0; only opt-in HTJ2K (`--htj2k --lossless`) codestream bytes change (the fix). After it, all 49 CID22 images and the medical corpus round-trip bit-exact and OpenJPH/Grok/Kakadu decode J2KSwift's HTJ2K output bit-exactly. See [RELEASE_NOTES_v10.24.1.md](RELEASE_NOTES_v10.24.1.md).
**Previous Release**: 10.24.0 (`J2KDICOMHelpers` Phase 3.1 — uncompressed pixel extraction)
