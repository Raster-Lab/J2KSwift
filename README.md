...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.4.0 ships **Phase 1 of partial-resolution decode**: `J2KDecoder.decodeResolution(_:options:)` now actually works (was throwing `notImplemented` since v6.x). Phase 1 is decode-then-downsample — correctly-dimensioned thumbnails / preview output at any of 6 resolution levels (0=lowest..5=full), no perf upgrade yet (full-decode + power-of-2 block-average). Phase 2 (true partial decode with code-block filter + iDWT truncation, projected ~13× thumbnail speedup) is the next investment. Default `decode()` behaviour unchanged; codestream bytes byte-identical to v10.3.0.
**Previous Release**: 10.3.0 (MG mammography decode tied with Kakadu on M2 via two default-flips)
