...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.24.0 ships **`J2KDICOMHelpers` Phase 3.1 — uncompressed pixel extraction**. Closes the Phase 3.1 known limitation from v10.21.0: previously `J2KDICOMFileParser.parse(_:)` returned pixel data only for J2K-tagged transfer syntaxes (`.j2kCompressed(metadata, pixelDataBytes)`) and metadata-only for the rest (`.uncompressed(metadata)`). v10.24.0 ships a SIBLING method `J2KDICOMFileParser.parseExtractingPixelData(_:)` that returns the richer `J2KDICOMFileWithPixelData` type — both cases now carry `pixelDataBytes`. For uncompressed inputs, the bytes are sliced from the `(7FE0,0010)` element in source byte order (no endianness conversion, no mosaic/planar-config interpretation) with explicit truncation detection when declared length < metadata-expected (`rows × columns × samplesPerPixel × bytesPerSample × numberOfFrames`). Existing `parse(_:)` + `J2KDICOMFile` unchanged — consumers pick whichever shape fits their workflow. MINOR / additive — no behaviour change on existing paths, codestream bytes byte-identical to v10.23.0.
**Previous Release**: 10.23.0 (encoder format-flexibility — encodeFile / encodeToFormat / public J2KFileWriter.wrap)
