...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.7.0 ships **ROI decode Stage 2 — tile-granular skip**: `decodeRegion(.direct)` now skips the *entire* decode (entropy + dequant + inverse DWT + colour + DC) for tiles lying completely outside the requested region. On the 2×2-tiled 16.8 MP MG mammography fixture, a viewport-zoom ROI within one quadrant skips the other three tiles wholesale (MG 3518×4784, 256² corner region: 84.70 → 49.32 ms, 1.72×). Default `decode()` behaviour unchanged; codestream bytes byte-identical to v10.6.0.
**Previous Release**: 10.6.0 (true ROI decode Stage 1 — entropy-skip for off-region code-blocks)
