...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.6.0 delivers the **ROI half** of the partial-decode arc: `J2KDecoder.decodeRegion(_:options:)` with the `.direct` strategy now does **true region-of-interest decode** — a spatial code-block filter skips the dominant entropy stage for every code-block outside the requested region (DX 2800×2288, 256² region: 46.05 → 33.21 ms, 1.39×). Also fixes a latent bug where the crop helper was 8-bit-only, silently corrupting `.fullImageExtraction` output for 16-bit medical images. Default `decode()` behaviour unchanged; codestream bytes byte-identical to v10.5.0.
**Previous Release**: 10.5.0 (true partial-resolution decode — 3-8× thumbnail speedup)
