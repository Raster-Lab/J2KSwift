...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.9.0 fixes a **Part-1 conformance defect** — J2KSwift's decoder silently mis-decoded any multi-layer JPEG 2000 codestream (the packet loop was hardcoded single-layer) — and ships **`decodeQuality`**, the last of the four partial-decode APIs. `decode()` now correctly handles multi-layer codestreams; `decodeQuality(layer:L)` reconstructs from quality layers `0...L`. Single-layer `decode()` is byte-exact unchanged; codestream bytes byte-identical to v10.8.0.
**Previous Release**: 10.8.0 (decodePartial umbrella API)
