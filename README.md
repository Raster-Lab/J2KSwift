...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.21.0 ships **`J2KDICOMHelpers` Phase 3 — DICOM file parser**. Library consumers can now parse `.dcm` file bytes natively to extract Transfer Syntax UID + Image Pixel Module metadata + (for J2K-tagged transfer syntaxes) the encapsulated Pixel Data byte stream — completing the DICOM read-side story. Combined with v10.17.0 (Phase 1 UID-and-config bridge) + v10.19.0 (Phase 2 Pixel Data encapsulation), the end-to-end flow from raw `.dcm` bytes to decoded `J2KImage` is now self-contained inside J2KSwift. New public API: `J2KDICOMFileParser.parse(_:)` + `J2KDICOMFile` enum (`.j2kCompressed(metadata, pixelDataBytes)` / `.uncompressed(metadata)`) + `J2KDICOMFileMetadata` value struct + `J2KDICOMFileError` typed errors. ADR-004 compliant — no DICOM library dependency added; pure byte-level parsing per DICOM PS3.5 §7 + §6.2 + PS3.3 §C.7.6.3.1. MINOR / additive — no signature changes elsewhere, codestream bytes byte-identical to v10.20.0.
**Previous Release**: 10.20.0 (JP3D `preWarm()` symmetric completion)
