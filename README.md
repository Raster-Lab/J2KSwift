...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.2.0 ships an **opt-in single-kernel H+V fused inverse 5/3 Int Metal kernel** behind `J2K_METAL_IDWT_FUSED=1`, gated by `inverse53IntFusedPixelThreshold = 12_000_000` so only ≥12 MP per-level passes take the fused path (MG-class mammography). The kernel collapses the v10.1.0 tiled pair (3 dispatches per level) into one dispatch by holding H-pass output in threadgroup memory across the V lift — eliminates the colLow/colHigh device round-trip. **Default decode behavior unchanged from v10.1.0** (codestream bytes byte-identical, decoder-only optimisation under an opt-in env flag). Variance bench on MG-class (10 interleaved trials): median lift +2.6 to +7.7 ms across all 3 MG fixtures.
**Previous Release**: 10.1.0 (tiled Metal inverse 5/3 DWT kernels default-on — closes the iDWT bottleneck)
