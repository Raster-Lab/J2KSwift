...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.3.0 closes the M2 MG mammography decode gap to Kakadu via **two coordinated default-flips**: (1) `DecoderPipeline._gpuHTEntropyEnabled` flipped ON → OFF (v6.2.0 D4 default was correct then but post-v10.0.0 NEON HT default-on, the multi-tile GPU HT entropy path is now slower than CPU+NEON on MG-class) and (2) `J2KMetalDWT.inverse53IntFusedEnabled` flipped OFF → ON with the v10.2.0 12 MP pixel-threshold retained. Combined MG decode wall reduction: **−20 to −29 ms (−25%)**. Kakadu gap on MG: 1.45× (v10.2.0) → **1.01-1.08× (tied on small/mid)**. Codestream bytes byte-identical to v10.2.0.
**Previous Release**: 10.2.0 (opt-in fused H+V inverse 5/3 Int Metal kernel for ≥12 MP fixtures)
