...
**Current Version**: 10.23.0
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.23.0 ships **encoder format-flexibility** — write-side counterpart to v10.22.0's decoder ship. Adds `J2KEncoder.encodeFile(_:to:format:)` + `J2KEncoder.encodeToFormat(_:format:)` extensions (in the `J2KFileFormat` module) that encode an image with the encoder's rich `J2KEncodingConfiguration` (HTJ2K, multi-tile, custom quality layers, etc.) AND wrap in the requested file format's box structure — without the re-encoding that `J2KFileWriter.write` does via the simpler `J2KConfiguration`. Plus new public `J2KFileWriter.wrap(codestream:headerForImage:)` for consumers who want to wrap a pre-encoded codestream without re-encoding (useful for splitting encode vs format-target across multiple wrappers). Existing `J2KFileWriter.write` semantics preserved — the new APIs are opt-in additive paths for richer control. MINOR / additive — no behaviour change on existing paths, codestream bytes byte-identical to v10.22.0.
**Previous Release**: 10.22.0 (decoder format-flexibility — decodeAnyFormat / decodeFile / public extractCodestream)
