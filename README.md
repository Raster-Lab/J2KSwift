...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.5.0 completes **true partial-resolution decode**: `J2KDecoder.decodeResolution(_:options:)` filters code-blocks by decomposition level before entropy decode and truncates the inverse DWT at the target level, producing reduced-dimension output directly. **Thumbnail decode is now 3-8× faster than full decode** (CT 512² 7.69×, PX 5.20×, DX 4.91×, MG 3.08×). Default `decode()` behaviour unchanged; codestream bytes byte-identical to v10.4.0.
**Previous Release**: 10.4.0 (partial-resolution decode Phase 1 — decode-then-downsample API)
