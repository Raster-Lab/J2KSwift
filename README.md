...
**Status**: Apple Silicon-first JPEG 2000 / HTJ2K (Part-15) implementation. v10.8.0 ships the **`decodePartial` umbrella API** — a single call (`J2KPartialDecodingOptions`) composing partial-resolution decode (v10.5.0, 3-8×), region-of-interest decode (v10.6.0/v10.7.0), and component-subset selection, including combining resolution + region. It was the last `notImplemented` partial-decode stub; `decodeResolution` + `decodeRegion` + `decodePartial` are now all implemented. Default `decode()` behaviour unchanged; codestream bytes byte-identical to v10.7.0.
**Previous Release**: 10.7.0 (ROI decode Stage 2 — tile-granular skip)
