...
final class MJ2PlayerTests: XCTestCase {
    ...
    func testPlaybackErrorTypes() {
        _ = MJ2PlaybackError.invalidFile
        _ = MJ2PlaybackError.noVideoTracks
        _ = MJ2PlaybackError.notInitialized
        _ = MJ2PlaybackError.seekFailed
        _ = MJ2PlaybackError.stopped

        let dummyError = NSError(domain: "test", code: 0)
        _ = MJ2PlaybackError.decodeFailed(frameIndex: 0, error: dummyError)
    }

    // MARK: - Playback State Enum Tests

    func testPlaybackStateEnum() {
        _ = MJ2PlaybackState.stopped
        _ = MJ2PlaybackState.playing
        _ = MJ2PlaybackState.paused
        _ = MJ2PlaybackState.seeking
    }

    // MARK: - Playback Mode Enum Tests

    func testPlaybackModeEnum() {
        _ = MJ2PlaybackMode.forward
        _ = MJ2PlaybackMode.reverse
        _ = MJ2PlaybackMode.stepForward
        _ = MJ2PlaybackMode.stepBackward
    }
