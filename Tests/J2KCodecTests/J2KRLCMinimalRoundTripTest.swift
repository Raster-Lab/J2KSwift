...
final class J2KRLCMinimalRoundTripTest: XCTestCase {
    ...
    func testSingleColumnRLC() throws {
        ...
        let (defData, defPC, defZBP, defSL, _, _, _) = try defCoder.encode(coefficients: coefficients, bitDepth: bitDepth)
    ...
    func testDefaultTermination() throws {
        ...
        let (isoData, isoPC, isoZBP, isoSL, _, _, _) = try isoCoder.encode(coefficients: coefficients, bitDepth: bitDepth)
