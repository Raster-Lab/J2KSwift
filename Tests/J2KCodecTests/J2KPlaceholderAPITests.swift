...
final class J2KPlaceholderAPITests: XCTestCase {
    ...
    func testEncoderEncodeGrayscaleImage() async throws {
    ...
    func testEncoderEncodeStartsWithSOC() async throws {
        let config = J2KEncodingConfiguration(
            quality: 0.9, lossless: false, decompositionLevels: 2
        )
        let encoder = J2KEncoder(encodingConfiguration: config)
    ...
    func testEncoderEncodeEndsWithEOC() async throws {
        let config = J2KEncodingConfiguration(
            quality: 0.9, lossless: false, decompositionLevels: 2
        )
        let encoder = J2KEncoder(encodingConfiguration: config)
    ...
    func testEncoderEncodeWithCustomConfig() async throws {
        let config = J2KEncodingConfiguration(
            quality: 0.8,
            lossless: false,
    ...
    func testDecoderDecodeThrowsOnInvalidData() async throws {
        let decoder = J2KDecoder()
        let data = Data()

        do {
            _ = try await decoder.decode(data)
            XCTFail("Expected J2KError.invalidData")
        } catch let error as J2KError {
            guard case .invalidData(_) = error else {
    ...
    func testDecoderDecodeThrowsOnMalformedCodestream() async throws {
        let decoder = J2KDecoder()
        ...
        do {
            _ = try await decoder.decode(data)
            XCTFail("Expected J2KError.decodingError")
        } catch let error as J2KError {
