// J2KHTMELCoderConformantTests.swift
...
final class HTMELCoderConformantTests: XCTestCase {

    // MARK: - Round-trip

    func testRoundTripAllZeros() {
        let events = [Bool](repeating: false, count: 100)
        let encoded = HTMELCoderConformant.encodeEvents(events)
        let decoded = HTMELCoderConformant.decodeEvents(encoded, count: events.count)
    ...
    func testStateAdvancesOnZeros() {
        ...
        let encoded = HTMELCoderConformant.encodeEvents(zeros)
