// V740NeonRefillDXWallBenchmark.swift
//
// v7.4 NEON refill — end-to-end DX 2800×2288 in-process wall-time
// A/B (scalar vs batched). The acceptance criterion is "DX in-process
// decode improves by at least 3 ms" before flipping the
// `neonRefillEnabled` default to `true`.

import XCTest
@testable import J2KCore
@testable import J2KCodec

final class V740NeonRefillDXWallBenchmark: XCTestCase {
    ...
    func testDXInProcessWall_ScalarVsBatchedRefill() async throws {
        ...
        print("=== v7.4 DX in-process MagSgn refill A/B (median of \(runs)) ===")
        print(String(format: "  Scalar refill:   %.2f ms", scalar))
        print(String(format: "  Batched refill:  %.2f ms", batched))
        print(String(format: "  Δ:               %.2f ms (%+.1f %%)", delta, pct))
        print()
        if delta >= 3.0 {
            print("  ✓ ≥ 3 ms threshold met — batched refill should default ON.")
        } else if delta >= 0 {
            print("  ⚠ < 3 ms threshold — per v7.4 spec acceptance criterion,")
            print("    keep batched refill behind a flag (default OFF).")
        } else {
