...
final class J2KRLCTraceTest: XCTestCase {
    ...
    func testOldCodeBehaviorLL() throws {
        ...
        let (encodedHL, passCountHL, zeroBitPlanesHL, segHL, _, _, _, _, _) = try coderHL.encode(
            coefficients: coefficients,
            bitDepth: bitDepth
        )
        
        let coderLL = BitPlaneCoder(width: width, height: height, subband: .ll)
        let (encodedLL, passCountLL, zeroBitPlanesLL, segLL, _, _, _, _, _) = try coderLL.encode(
            coefficients: coefficients,
            bitDepth: bitDepth
        )
