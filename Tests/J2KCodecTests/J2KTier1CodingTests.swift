...
final class J2KBitPlaneCoderTests: XCTestCase {
    ...
    func testBitPlaneCoderEncodeZeros() throws {
        ...
        let (data, passCount, zeroBitPlanes, _, _, _, _, _, _) = try coder.encode(coefficients: coefficients, bitDepth: 8)
    ...
    func testBitPlaneEncodeZeros() throws {
        ...
        let (data, passCount, zeroBitPlanes, _, _, _, _, _, _) = try encoder.encode(coefficients: original, bitDepth: bitDepth)
...
final class J2KBypassModeTests: XCTestCase {
    ...
    func testBitPlaneCoderBypassVsNormal() throws {
        ...
        let (normalData, normalPasses, normalZero, _, _, _, _, _, _) = try normalCoder.encode(
            coefficients: coefficients,
            bitDepth: bitDepth
        )
        ...
        let (bypassData, bypassPasses, bypassZero, _, _, _, _, _, _) = try bypassCoder.encode(
            coefficients: coefficients,
            bitDepth: bitDepth
        )
...
final class J2KTerminationModeTests: XCTestCase {
    ...
    func testBitPlaneCoderDefaultTermination() throws {
        ...
        let (data, passCount, _, _, _, _, _, _, _) = try coder.encode(coefficients: coefficients, bitDepth: 8)
    ...
    func testBitPlaneCoderPredictableProducesMoreBytes() throws {
        ...
        let (defaultData, _, _, _, _, _, _, _, _) = try defaultCoder.encode(coefficients: coefficients, bitDepth: bitDepth)
        ...
        let (predictableData, _, _, _, _, _, _, _, _) = try predictableCoder.encode(coefficients: coefficients, bitDepth: bitDepth)
