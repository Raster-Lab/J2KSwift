// v10.3-research Phase 2-1 — Metal inverse 5/3 Int 2D-layout parity oracle.
//
...
// same synthetic input — once with `inverse53Int2DLayoutEnabled = false`
// (scalar production path) and once with `= true` (new 2D path).
//
...
final class V10_3_MetalIDWTInverse532DParityTests: XCTestCase {
    ...
    private func assertParity(
        ...
        let scalar = try await runDecode(dwt, chain: chain, use2D: false)
        let twod   = try await runDecode(dwt, chain: chain, use2D: true)
        XCTAssertEqual(scalar.count, twod.count,
                       "[\(spec.label)] output count mismatch")
        if scalar != twod {
            // Find first divergence.
            var first = -1
            for i in 0..<min(scalar.count, twod.count) where scalar[i] != twod[i] {
                first = i; break
            }
            XCTFail("[\(spec.label)] 2D output diverges from scalar at idx \(first); scalar=\(scalar[max(0,first)]) 2d=\(twod[max(0,first)])")
        }
    }
