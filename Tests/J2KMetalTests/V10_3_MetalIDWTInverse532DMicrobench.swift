// v10.3-research Phase 2-1 — Metal inverse 5/3 Int 2D-layout microbench.
//
// A/B times `J2KMetalDWT.inverse2DInt32MultiLevelFused` with the
// `inverse53Int2DLayoutEnabled` flag OFF (scalar production kernels)
// vs ON (new 2D-layout kernels). Same synthetic Int32 input, 5
...
final class V10_3_MetalIDWTInverse532DMicrobench: XCTestCase {
    ...
    private static func bench(
        ...
        let prev = J2KMetalDWT.inverse53Int2DLayoutEnabled
        J2KMetalDWT.inverse53Int2DLayoutEnabled = use2D
        defer { J2KMetalDWT.inverse53Int2DLayoutEnabled = prev }
        for _ in 0..<warmups {
    ...
    func testPhase2_1_2DLayoutMicrobench() async throws {
        let dwt = J2KMetalDWT()
        ...
        for spec in fixtures {
            ...
            let scalar = try await Self.bench(dwt, chain: chain, use2D: false,
                                              warmups: 3, iterations: 9)
            let twod = try await Self.bench(dwt, chain: chain, use2D: true,
                                            warmups: 3, iterations: 9)
        ...
        print("")
        print("| Fixture | Pixels | Scalar ms | 2D ms | Speedup | Δ ms |")
        print("|---|---:|---:|---:|---:|---:|")
        for r in rows {
        ...
        print("")
        // Phase 2-1 gate: 2D ≥ 1.5× faster on at least one of DX/MG.
        let dxRow = rows.first { $0.label.contains("2544") }!
        let mgRow = rows.first { $0.label.contains("3520") }!
        print(String(format: "Phase 2-1 gate (≥1.5x on DX or MG):"))
        ...
        if dxRow.speedup >= 1.5 || mgRow.speedup >= 1.5 {
            print("  Status: PASS — 2D layout clears Phase 2-1 gate. Proceed to Phase 2-2.")
