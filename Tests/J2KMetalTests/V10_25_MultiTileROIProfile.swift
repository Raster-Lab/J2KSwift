// V10_25_MultiTileROIProfile.swift
//
// v10.25-research Phase 0 — profile multi-tile ROI decode (MG
// 3518×4784, 256² centre region) to confirm per-tile iDWT is the
// dominant cost on the SURVIVING tile after v10.7.0's tile-skip.
// The v10.24 single-tile windowed iDWT was a wash on the production
// corpus because >3 MP fixtures default to multi-tile. Per-tile
// windowed iDWT (applied to the surviving tile) should win on MG.
...
final class V10_25_MultiTileROIProfile: XCTestCase {
    ...
    func testStage3Profile_MG3518_256region() async throws {
        ...
        V10_25 Phase 0 — MG 3518×4784 (multi-tile, 2x2 grid), 256² centre ROI, n=%d trials, release

        Per-call wall: %.2f ms (v10.7.0 baseline ~49 ms after 3-tile skip)

        Stage breakdown (per-call, accumulator-derived, ACROSS ALL TILES):
          extractTileData :  %5.2f ms  (%.1f%%)
          entropyDecoding :  %5.2f ms  (%.1f%%)
          dequantization  :  %5.2f ms  (%.1f%%)
          iDWT            :  %5.2f ms  (%.1f%%)  ← v10.25 PER-TILE WINDOWED TARGET
        ...
        For the 1 surviving tile (~1760×2392, ~4.2 MP), windowed iDWT
        on a small in-tile region should cut its iDWT cost from
        ~iDWT ms to <1 ms — saving substantial wall.
        """,
            trials, avgWall,
