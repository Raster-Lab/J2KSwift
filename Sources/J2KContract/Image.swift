...
public final class ImageDestination: Sendable {
    ...
    /// Asynchronous counterpart of `write`, for codecs whose decode suspends.
    ///
    /// The closure receives the **owner and its lease**, never a pointer.
    /// MEM-08 forbids a borrow spanning an `await`, so an `async` codec
    /// cannot be handed a buffer: it retains the owner across suspension and
    /// borrows synchronously inside its own final-output stage. MEM-06 as
    /// revised in contract 0.6.0 states this directly — "an entry point whose
    /// parameter is a pointer cannot satisfy that and is not a conforming
    /// shared-storage API, whatever its internals do".
    ///
    /// Lifecycle is identical to `write`: one use, sealed on success,
    /// invalidated on any throw or cancellation so nothing partial is
    /// published.
    public func writeAsync(
        _ body: (any WritableImageStorage, StorageWriteLease) async throws -> Void
    ) async throws -> Image {
        let accepted = started.withLock { value in
            guard !value else { return false }
            value = true
